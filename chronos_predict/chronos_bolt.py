import pandas as pd
import os
from func_chronos_bolt import *

ALL_PREDICT = True
DO_TRAIN = True
ZERO_SHOT = False


def main():
    print("=== Loading Data ===")
    required_cols = ['書名', '日付', 'POS販売冊数']
    df = pd.read_parquet('data/df_for.parquet', columns=required_cols)
    print("Complete!")

    adapter_path = None

    if ZERO_SHOT:
        print("Zero-shot mode: skipping adapter.")
    elif DO_TRAIN:
        adapter_path = train_model(df)
    else:
        possible_path = os.path.join(CONFIG["lora_output_dir"], "final_adapter")
        if os.path.exists(possible_path):
            adapter_path = possible_path
            print(f"Using existing adapter: {adapter_path}")
        else:
            print(f"Adapter not found at {possible_path}. Falling back to zero-shot inference.")

    suffix = "(zero-shot)" if not adapter_path else ""
    CONFIG["output_dir"] = f"chronos_bolt{suffix}"

    print(f"\n=== Starting Inference ===")
    decile_books = extract_decile_books(df)

    model = load_model(adapter_path)

    infer_ds = ChronosBoltDataset(
        df=df, prediction_length=CONFIG["prediction_length"],
        mode="inference", context_length=CONFIG["context_length"],
        decile_books=decile_books, all_predict=ALL_PREDICT
    )
    print(f"Prepared {len(infer_ds)} valid samples for inference.")

    forecasts = run_inference(model, infer_ds)
    save_results(infer_ds.metadata, forecasts, decile_books, ALL_PREDICT)
    evaluate_predictions(infer_ds.metadata, forecasts, ALL_PREDICT)
    print("Inference and saving complete.")


if __name__ == "__main__":
    main()
