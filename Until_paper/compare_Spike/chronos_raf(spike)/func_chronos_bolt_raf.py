...
def _compute_spike_mask_for_targets(
    ...
    for full_i in range(start, end):
        # Need enough past to form a window
        if full_i < context_length:
            mask[full_i - start] = False
            continue

        past = full[full_i - context_length: full_i]  # excludes current
...
def evaluate_spike_predictions(
    ...
    for i, s in enumerate(samples):
        ...
        # Per-book metrics (mean over days in the subset)
    ...
    # Aggregate: mean over books (ignore NaN books with 0 spike / 0 non-spike)
    agg = {
