...
def plot_improvement(all_metrics, models, colors, markers, quantile_col, output_dir, mode='spike'):
    ...
    baseline = next(m[key] for m, name in zip(all_metrics, models) if name == 'LSTM-noRAF')
    improvements = []
...
for spike_z in SPIKE_ZSCORES:
    for quantile in QUANTILES:
        ...
        baseline_spike = next(m['MAE_spike'] for m, name in zip(all_metrics, models) if name == 'LSTM-noRAF')
