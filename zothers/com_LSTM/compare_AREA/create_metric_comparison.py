...
raw_metrics = ['Area_over', 'Area_under', 'Area_total']

label_map = {'Area_over': 'Area (predict over actual)',
             'Area_under': 'Area (predict under actual)',
             'Area_total': 'Area (total |pred - actual|)'}
...
def compute_metrics(df, quantile_col):
    ...
    for _, grp in df.groupby('book_name'):
        ...
        diff = grp[quantile_col].values - grp['actual'].values
        days = grp['day'].values
        overs.append(np.trapezoid(np.where(diff > 0, diff, 0), days))
        unders.append(np.trapezoid(np.where(diff < 0, -diff, 0), days))
    return {'MAE': df['ae'].mean(), 'RMSE': np.sqrt(df['se'].mean()),
            'Area_over': np.mean(overs), 'Area_under': np.mean(unders),
            'Area_total': np.mean([o + u for o, u in zip(overs, unders)])}
