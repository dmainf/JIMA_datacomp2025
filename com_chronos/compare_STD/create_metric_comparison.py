...
STD_BINS = list(range(0, 145, 5)) + [np.inf]
STD_BIN_LABELS = [f'{i}-{i+5}' for i in range(0, 140, 5)] + ['140+']
...
def build_binned_stats(dfs, models, quantile_col):
    ...
    for df, model_name in zip(dfs, models):
        ...
        for bin_label in STD_BIN_LABELS:
            ...
            if len(subset) == 0:
                continue
            subset_full = df[df['std_bin'] == bin_label]
...
def plot_binned_boxplot(dfs, models, quantile_col, output_dir):
    ...
    for i, (df, model_name) in enumerate(zip(dfs, models)):
        ...
        data_by_bin = [df.loc[df['std_bin'] == b, 'mape'].dropna().values for b in bin_order]
    ...
    ax.set_title(f'MAPE distribution per std bin  [{quantile_col}]', fontsize=14, fontweight='bold')
...
for quantile in QUANTILES:
    ...
    for _, row in first_model.iterrows():
        print(f"  {row['std_bin']}: {row['n']} rows, {row['n_books']} books")
