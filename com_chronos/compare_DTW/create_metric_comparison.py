...
raw_metrics = ['DTW', 'DTW_temporal', 'DTW_shape']

label_map = {'DTW': 'DTW distance',
             'DTW_temporal': 'DTW temporal shift (mean |i-j|)',
             'DTW_shape': 'DTW shape dissimilarity (normalized)'}
...
def compute_metrics(df, quantile_col):
    ...
    dtw_list, temporal_list, shape_list = [], [], []
    for _, grp in df.groupby('book_name'):
        grp = grp.sort_values('day')
        actual = grp['actual'].values
        pred   = grp[quantile_col].values
        path, dist = dtw_path(actual, pred)
        path = np.array(path)
        dtw_list.append(dist)
        temporal_list.append(np.mean(np.abs(path[:, 0] - path[:, 1])))
        shape_list.append(dist / len(path))
    return {'DTW': np.mean(dtw_list),
            'DTW_temporal': np.mean(temporal_list),
            'DTW_shape': np.mean(shape_list)}
...
def generate_latex_table(all_quantile_metrics, models, quantiles, metrics):
    ...
    metric_header = {'DTW': 'DTW', 'DTW_temporal': 'Temporal', 'DTW_shape': 'Shape'}
    ...
    col_spec = 'l' + ''.join(['r' * len(metrics)] * len(quantiles))

    lines = []
    lines.append(r'\begin{table*}[t]')
    lines.append(r'  \caption{各モデルおよび分位点レベルにおける DTW 距離・時間ずれ・形状乖離の比較．}')
    lines.append(r'  \label{tab:dtw_comparison}')
    ...
    for q in quantiles:
        for m in metrics:
            header_m += f' & \\multicolumn{{1}}{{c}}{{{metric_header.get(m, m)}}}'
    header_m += r' \\'
