...
raw_metrics = ['MAE', 'RMSE', 'MASE']

label_map = {'MAE': r'MAE', 'RMSE': r'RMSE', 'MASE': r'MASE'}
...
def compute_metrics(df, quantile_col):
    ...
    for _, grp in df.groupby('book_name'):
        ...
        naive_mae = np.abs(np.diff(grp['actual'].values)).mean()
        if naive_mae > 0 and np.isfinite(naive_mae):
            mase_per_book.append(grp['ae'].mean() / naive_mae)
    return {'MAE': df['ae'].mean(), 'RMSE': np.sqrt(df['se'].mean()),
            'MASE': np.nanmean(mase_per_book)}
